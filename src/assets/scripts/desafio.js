...
.controller('DesafioCtrl', ($scope, Table, Score) => {
  ...
  $scope.generateChallenge = (score, challengeId) => {
    $scope.error = '';
    ...
    if (Number(teamName[0]) <= 0 || Number(teamName[0]) > 2 || isNaN(Number(teamName[0]))) {
      $scope.error = `Não foi possível identificar o nível a partir do nome da equipe.
      Use o formato "1 - equipe" ou "2 - equipe" para indicar o nível 1 ou 2.`
      return;
    } else {
      teamNivel = Number(teamName[0]);
    }

    const ok = confirm(`Tem certeza de que deseja gerar o desafio ${challengeId} da equipe ${teamName}?`);

    if (!ok) {
      return;
    }

    const possibleChallenges = without(window['DES' + teamNivel], score.desafio1);
    const challenge = possibleChallenges[Math.floor(Math.random() * possibleChallenges.length)];
    ...
    Score.update(newChallenges, (score) => { 
  ...
  (() => {
    ...
    window[upper+1] = window[(a + z + d + 1).toUpperCase()]['map']((e) => Tokens.decode(Tokens.decode(e)));
    window[upper+2] = window[(a + z + d + 2).toUpperCase()]['map']((e) => Tokens.decode(Tokens.decode(e)));
  })();
})

function makeAll(chals) {
  chals = chals.map((e) => Tokens.encode(Tokens.encode(e)));
