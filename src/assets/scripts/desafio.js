...
angular
  ...
  .controller('DesafioCtrl', ($scope, Table, Score) => {
    ...
    $scope.generateChallenge = (score, challengeId) => {
      ...
      const teamName = score.team && score.team.name;
      if (!teamName) {
        $scope.error =
          'O nome desta equipe não está definido, verifique se a equipe possui um nome válido';
        return;
      }
      ...
      const ok = confirm(
        `Tem certeza de que deseja gerar o desafio ${challengeId} da equipe ${teamName}?`
      );
      ...
      const possibleChallenges = without(
        window['DES' + teamNivel],
        score.desafio1
      );
      const challenge =
        possibleChallenges[
    ...
    (() => {
      ...
      window[upper + 1] = window[(a + z + d + 1).toUpperCase()]['map'](e =>
        Tokens.decode(Tokens.decode(e))
      );
      window[upper + 2] = window[(a + z + d + 2).toUpperCase()]['map'](e =>
        Tokens.decode(Tokens.decode(e))
      );
    })();
  });
