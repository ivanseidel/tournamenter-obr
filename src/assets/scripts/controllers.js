	...
	.controller('SaveScoreController',
		($scope, $modalInstance, extra, Table) => {
			...
			$scope.ok = () => {
				...
				const index2 = (SCORE_WITH_TIME ? selected.round * 2 + 1 : selected.round);
				const isScoredAlready = score[index1] || (SCORE_WITH_TIME && score[index2]);
			...
			$scope.selectScoredTeam = (doNotSave) => {
				...
				modalInstance.result.then(selected => {
					if (doNotSave && selected && selected.score.id) {
						...
						var index = SCORE_WITH_TIME ? round * 2 : round;
					...
					if (!doNotSave && selected && selected.score.id) {
						...
						function checkScore() {
							...
							Score.get({
								...
								console.log(score.scores[number2].value, Math.round(time));
								if (score.scores[number1].value != total){
									window.alert('FALHA AO SALVAR #3. REFAÇA A OPERAÇÃO.');
								} else if (SCORE_WITH_TIME && score.scores[number2].value != Math.floor(time)){
