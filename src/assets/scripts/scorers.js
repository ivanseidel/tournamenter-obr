  ...
  .factory('RescueScorer2018Nacional', function () {
    ...
    const scorings = {
      ...
      victims_alive: function (sub, val, scorings, model) {
        const salvou_todas_vivas =
          model.victims.total <= model.victims_alive.total;
        const tentativa = model.victims.attempts - 1;
        const nivel_alto = model.altura.final;
        const pontos_base = nivel_alto ? 40 : 30;
        const pontos = val * Math.max(pontos_base - 5 * tentativa, 0);
      ...
      victims_dead: function (sub, val, scorings, model) {
        ...
        const tentativa = model.victims.attempts - 1;
        const nivel_alto = model.altura.final;
        let pontos_base = nivel_alto ? 30 : 20;
      ...
      score: function (model) {
        ...
        for (const k in model) {
          ...
          for (const i in group) {
            ...
            const points = scorings[k][mission];
  ...
  .factory('FLLWorldClassScorer', function () {
    ...
    return {
      ...
      score: function (model) {
        ...
        for (const mission in model) {
          // console.log(mission);
          if (!scorings[mission]) {
            continue;
          }
          let score = model[mission];
          if (score === false) {
            score = 0;
          }
          if (score === true) {
          ...
          const points = scorings[mission][score];
        ...
        scored.reverseReplicate = [0, 15][
          model.reverseReplicate * model.reverseBasker
        ];
        ...
        scored.gyros = 0;
        if (model.engage) {
          let percentage = (model.gyros ? model.gyros * 1 + 9 : 0) / 100.0;
